(function () {
  ...
  function buildRamCapacityEnvelopeSvg(result) {
    ...
    const pointSvg = points.map(function (point, index) {
      ...
      const yAxisLabels = [0, 0.25, 0.5, 0.75, 1].map(function(ratio) {

        const tickValue = maxY * ratio;

        const tickY = toY(tickValue);

        const precision = tickValue >= 10 ? 0 : 1;

        return '<text x="' + (plot.x - 12) + '" y="' + (tickY + 3).toFixed(1) + '" text-anchor="end" class="tick">' + svgText(gb(tickValue, precision)) + '</text>';
