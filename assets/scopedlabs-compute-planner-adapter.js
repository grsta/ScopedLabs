...
/* COMPUTE PLANNER START CTA WORKLOAD AWARE V1 */
(function () {
  "use strict";

  var VERSION = "scopedlabs-compute-planner-start-aware-001";

  function isPlannerPage() {
    return String(window.location && window.location.pathname || "").indexOf("/tools/compute/workload-planner/") !== -1;
  }
  ...
  function workloadId(workload) {
    return workload && (workload.id || workload.workloadId || workload.uid || workload.key) || "";
  }

  function workloadList(plan) {
    if (!plan) return [];
    ...
    if (plan.activeWorkload && (workloadId(plan.activeWorkload) || plan.activeWorkload.name)) return [plan.activeWorkload];
    return [];
  }

  function findSetupTarget() {
    ...
    if (node && node.closest) return node.closest("section, .card, .panel, form") || node;
    return document.querySelector("form") || document.querySelector("main") || document.body;
  }

  function focusSetupField(target) {
    var field = null;
    if (target && target.querySelector) {
      field = target.querySelector("input, select, textarea, button");
    }
  ...
  function promptForWorkloadSetup() {
    var target = findSetupTarget();
    if (!target) return;
  ...
  function resolveWorkloadRoute(plan, workloads) {
    var RouteEngine = window.ScopedLabsComputeGuidedRouteEngine;
    if (!RouteEngine || typeof RouteEngine.resolve !== "function") return null;

    var bestSummary = null;
    for (var i = 0; i < workloads.length; i += 1) {
      var workload = workloads[i];
      var id = workloadId(workload) || "workload-" + i;
      var context = {
        guidedFlow: true,
      ...
      try {
        ...
        if (!decision || !decision.nextHref) continue;
        if (Array.isArray(decision.remainingTools) && decision.remainingTools.length) return { decision: decision, workload: workload, workloadId: id };
        if (!bestSummary) bestSummary = { decision: decision, workload: workload, workloadId: id };
      } catch (error) {
        /* keep trying */
      }
    }
  ...
  function findStartCtas() {
    return Array.from(document.querySelectorAll("a, button")).filter(function (node) {
      var text = String(node.textContent || "").replace(/\s+/g, " ").trim();
      return /Start\s+Guided\s+Flow|Resume\s+Guided\s+Flow|Continue\s+Guided\s+Flow/i.test(text);
    });
  }

  function startGuidedFlowFor(workload, id) {
    var State = window.ScopedLabsComputePlanState || {};
    try {
      if (typeof State.startGuidedFlow === "function") {
        State.startGuidedFlow(id || workloadId(workload));
        return;
      }
    } catch (error) {
    ...
    try {
      window.localStorage.setItem("scopedlabs:pipeline:compute:guided-flow", JSON.stringify({
        guidedFlow: true,
        routeMode: "compute-guided",
        workloadId: id || workloadId(workload),
        activeWorkloadId: id || workloadId(workload),
        workload: workload
      }));
    } catch (error) {
      /* no-op */
  ...
  function refreshStartCtas() {
    if (!isPlannerPage()) return;
    var plan = readComputePlan();
    var workloads = workloadList(plan);
    var hasWorkload = workloads.length > 0;
    var route = hasWorkload ? resolveWorkloadRoute(plan, workloads) : null;

    findStartCtas().forEach(function (node) {
      node.hidden = false;
      node.style.display = "";
      node.removeAttribute("aria-hidden");
      node.removeAttribute("aria-disabled");
      if ("disabled" in node) node.disabled = false;
      node.textContent = "Start Guided Flow";
      node.setAttribute("data-compute-start-guided-flow", "workload-aware");

      if (!hasWorkload) {
        node.setAttribute("data-compute-start-guided-action", "setup");
        node.setAttribute("title", "Save a workload first. This button will take you back to the setup form.");
        node.removeAttribute("data-compute-start-guided-href");
        return;
      }

      node.setAttribute("data-compute-start-guided-action", "route");
      if (route && route.decision && route.decision.nextHref) {
        node.setAttribute("data-compute-start-guided-href", route.decision.nextHref);
        node.setAttribute("data-compute-start-guided-workload-id", route.workloadId || "");
        node.setAttribute("title", "Open the next incomplete Compute planning step.");
      }
    });
  }

  function handleStartClick(event) {
    var target = event.target && event.target.closest ? event.target.closest("[data-compute-start-guided-flow]") : null;
    if (!target || !isPlannerPage()) return;
    ...
    if (!workloads.length) {
      ...
      promptForWorkloadSetup();
      return;
    }

    var route = resolveWorkloadRoute(plan, workloads);
    if (!route || !route.decision || !route.decision.nextHref) return;

    event.preventDefault();
    event.stopPropagation();
    if (typeof event.stopImmediatePropagation === "function") event.stopImmediatePropagation();
    startGuidedFlowFor(route.workload, route.workloadId);
    window.location.assign(route.decision.nextHref);
  }
  ...
  function startObserver() {
    ...
    var observer = new MutationObserver(function () { refreshStartCtas(); });
    observer.observe(document.documentElement, { childList: true, subtree: true });

    window.addEventListener("storage", refreshStartCtas);
    window.addEventListener("scopedlabs:compute:plan-change", refreshStartCtas);
    window.addEventListener("scopedlabs-compute-plan-change", refreshStartCtas);
    document.addEventListener("click", handleStartClick, true);
  }

  window.ScopedLabsComputePlannerStartCta = Object.freeze({
    version: VERSION,
    refresh: refreshStartCtas,
    workloadList: workloadList,
    promptForWorkloadSetup: promptForWorkloadSetup
  });
