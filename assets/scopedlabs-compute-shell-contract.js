...
// compute-shell-power-thermal-active-workflow-0709
(function () {
  if (window.__ScopedLabsComputePowerThermalActiveWorkflow0709) return;
  window.__ScopedLabsComputePowerThermalActiveWorkflow0709 = true;

  var STYLE_ID = "scopedlabs-compute-power-thermal-active-workflow-0709";
  var RESULT_KEY = "scopedlabs:pipeline:compute:power-thermal";
  var VM_RESULT_KEY = "scopedlabs:pipeline:compute:vm-density";

  function isPowerThermalPage() {
    return !!(document.body && document.body.getAttribute("data-step") === "power-thermal");
  }
  ...
  function injectPowerThermalWorkflowStyle() {
    if (!isPowerThermalPage() || document.getElementById(STYLE_ID)) return;

    var style = document.createElement("style");
    style.id = STYLE_ID;
    style.textContent = [
      'body[data-step="power-thermal"] #flow-note { display: none !important; visibility: hidden !important; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-card { margin-top: 16px; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-card .eyebrow { color: #3fff80; font-weight: 500; letter-spacing: 0.04em; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 8px; margin-top: 12px; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-chip { border: 1px solid rgba(63, 255, 128, 0.14); border-radius: 10px; padding: 10px 12px; background: rgba(1, 18, 12, 0.55); min-height: 48px; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-chip .mini-label { display: block; font-size: 0.72rem; letter-spacing: 0.08em; text-transform: uppercase; color: var(--sl-muted, #9fb4ad); margin-bottom: 4px; }',
      'body[data-step="power-thermal"] .power-thermal-active-workflow-chip strong { display: block; font-size: 0.86rem; }',
      '@media (max-width: 760px) { body[data-step="power-thermal"] .power-thermal-active-workflow-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }'
    ].join("\n");

    document.head.appendChild(style);
  }

  function findPowerThermalKbCard() {
    return document.getElementById("scopedlabs-help") || Array.from(document.querySelectorAll("section.card, div.card")).find(function (card) {
      var text = textOf(card);
      return text.indexOf("Power") >= 0 && text.indexOf("Thermal") >= 0 && text.indexOf("Open KB Guide") >= 0;
    }) || null;
  }
  ...
  function selectedText(id, fallback) {
    ...
    if (el.tagName === "SELECT") {
      ...
      return selected && selected.textContent ? textOf(selected) : fallback;
    }
  ...
  function readJsonStorage(key) {
    try {
      return JSON.parse(window.sessionStorage.getItem(key) || window.localStorage.getItem(key) || "null");
    } catch (error) {
      return null;
    }
  }
  ...
  function readActiveWorkflowContext() {
    ...
    try {
      if (typeof State.buildWorkloadDisplayContext === "function") {
        return State.buildWorkloadDisplayContext("Power / Thermal");
      }
    } catch (error) {}
    return null;
  }
  ...
  function contextRowValue(context, label, fallback) {
    ...
    for (var i = 0; i < rows.length; i += 1) {
      var row = rows[i] || {};
      var rowLabel = String(row.label || row.name || row.key || "").toLowerCase();
      if (rowLabel === wanted && row.value != null && row.value !== "") return String(row.value);
    }
  ...
  function workflowData() {
    var context = readActiveWorkflowContext();
    var result = readJsonStorage(RESULT_KEY) || {};
    var vmResult = readJsonStorage(VM_RESULT_KEY) || {};
    var status = result.status || result.localStatus || result.recommendationStatus || "Pending Calculation";

    var environment = contextRowValue(context, "Environment", "Active workload");
    var workloadType = contextRowValue(context, "Workload Type", contextRowValue(context, "Workload Mix", "Compute workload"));
    var plannedNodes = inputValue("nodes", "", vmResult.plannedHosts ? String(vmResult.plannedHosts) : "10");

    return {
      title: context && context.hasActiveWorkload ? context.title : "Active Workflow",
      summary: context && context.hasActiveWorkload ? context.description : environment + " | " + workloadType + " | Power / Thermal",
      environment: environment,
      workloadType: workloadType,
      demandSource: "VM Density",
      nextTool: "Compute Summary",
      plannedNodes: plannedNodes,
      rackLimit: inputValue("rackKw", " kW", "5 kW"),
      coolingCapacity: inputValue("coolingTons", " tons", "3 tons"),
      status: status
    };
  }

  function buildPowerThermalWorkflowCard() {
    var card = document.createElement("section");
    card.className = "card power-thermal-active-workflow-card";
    card.setAttribute("data-power-thermal-active-workflow-card", "0709");
    card.setAttribute("data-compute-shell-owned-active-workflow", "0709");
    card.setAttribute("data-compute-planner-routing-context", "power-thermal-0709");

    card.innerHTML = [
      '<div class="eyebrow">ACTIVE WORKFLOW &rarr; POWER / THERMAL</div>',
      '<h2 class="h2" style="margin-top: 8px;" data-power-thermal-workflow-title>Active Workflow</h2>',
      '<p class="muted" style="margin-top: 4px;" data-power-thermal-workflow-summary>Power / Thermal uses the active workload context and carries rack/cooling readiness into Compute Summary.</p>',
      '<div class="power-thermal-active-workflow-grid" aria-label="Active workload context">',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Environment</span><strong data-power-thermal-workflow-value="environment">Active workload</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Workload Type</span><strong data-power-thermal-workflow-value="workloadType">Compute workload</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Demand Source</span><strong data-power-thermal-workflow-value="demandSource">VM Density</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Next Tool</span><strong data-power-thermal-workflow-value="nextTool">Compute Summary</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Planned Nodes</span><strong data-power-thermal-workflow-value="plannedNodes">10</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Rack Limit</span><strong data-power-thermal-workflow-value="rackLimit">5 kW</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Cooling Capacity</span><strong data-power-thermal-workflow-value="coolingCapacity">3 tons</strong></div>',
      '<div class="power-thermal-active-workflow-chip"><span class="mini-label">Status</span><strong data-power-thermal-workflow-value="status">Pending Calculation</strong></div>',
      '</div>'
    ].join("");
  ...
  function setWorkflowValue(card, key, value) {
    var node = card.querySelector('[data-power-thermal-workflow-value="' + key + '"]');
    if (node) node.textContent = value;
  }

  function updatePowerThermalWorkflowCard() {
    if (!isPowerThermalPage()) return;

    var card = document.querySelector('[data-power-thermal-active-workflow-card="0709"]');
    if (!card) return;

    var data = workflowData();
    var title = card.querySelector("[data-power-thermal-workflow-title]");
    var summary = card.querySelector("[data-power-thermal-workflow-summary]");

    if (title) title.textContent = data.title;
    if (summary) summary.textContent = data.summary;

    setWorkflowValue(card, "environment", data.environment);
    setWorkflowValue(card, "workloadType", data.workloadType);
    setWorkflowValue(card, "demandSource", data.demandSource);
    setWorkflowValue(card, "nextTool", data.nextTool);
    setWorkflowValue(card, "plannedNodes", data.plannedNodes);
    setWorkflowValue(card, "rackLimit", data.rackLimit);
    setWorkflowValue(card, "coolingCapacity", data.coolingCapacity);
    setWorkflowValue(card, "status", data.status);
  }

  function removeDuplicatePowerThermalWorkflowCards(keep) {
    Array.from(document.querySelectorAll("[data-power-thermal-active-workflow-card]")).forEach(function (card) {
      if (card !== keep && card.parentNode) card.parentNode.removeChild(card);
    });
  }

  function placePowerThermalWorkflowCard() {
    if (!isPowerThermalPage()) return false;

    var kbCard = findPowerThermalKbCard();
    if (!kbCard || !kbCard.parentNode) return false;

    var current = document.querySelector('[data-power-thermal-active-workflow-card="0709"]');
    if (!current) {
      current = buildPowerThermalWorkflowCard();
      kbCard.insertAdjacentElement("afterend", current);
    }

    removeDuplicatePowerThermalWorkflowCards(current);
    updatePowerThermalWorkflowCard();
    return true;
  }

  function bindPowerThermalWorkflowRefresh() {
    if (document.documentElement.dataset.powerThermalWorkflowRefresh0709 === "true") return;
    document.documentElement.dataset.powerThermalWorkflowRefresh0709 = "true";

    ["nodes", "rackKw", "coolingTons"].forEach(function (id) {
      var el = document.getElementById(id);
      if (!el) return;
      el.addEventListener("input", function () { window.setTimeout(updatePowerThermalWorkflowCard, 0); });
      el.addEventListener("change", function () { window.setTimeout(updatePowerThermalWorkflowCard, 0); });
    });

    document.addEventListener("click", function (event) {
      if (!event.target || !event.target.closest) return;
      if (event.target.closest("#calc") || event.target.closest("#reset")) {
        window.setTimeout(updatePowerThermalWorkflowCard, 80);
      }
    });
  }

  function applyPowerThermalWorkflowContract() {
    if (!isPowerThermalPage()) return;
    injectPowerThermalWorkflowStyle();
    bindPowerThermalWorkflowRefresh();
    placePowerThermalWorkflowCard();
  }

  function observePowerThermalWorkflowCard() {
    if (!isPowerThermalPage() || !window.MutationObserver || !document.body) return;

    var observer = new MutationObserver(function () {
      applyPowerThermalWorkflowContract();
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () {
      applyPowerThermalWorkflowContract();
      observePowerThermalWorkflowCard();
      window.setTimeout(applyPowerThermalWorkflowContract, 300);
      window.setTimeout(applyPowerThermalWorkflowContract, 900);
    });
  } else {
    applyPowerThermalWorkflowContract();
    observePowerThermalWorkflowCard();
