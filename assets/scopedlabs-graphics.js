 ...
 * Factory asset for report-safe SVG renderers.
 * Version: scopedlabs-graphics-001
 *
...
(function () {
  ...
  const VERSION = "scopedlabs-graphics-001";
  ...
  function fmtFt(value, digits = 1) {
    const n = Number(value);
    return Number.isFinite(n) ? n.toFixed(digits).replace(/\.0$/, "") + " ft" : "—";
  }
  ...
  function normalizeSegments(items, spanFt) {
    const span = Math.max(1, num(spanFt, 1));

    return (Array.isArray(items) ? items : [])
      .map((item) => {
        const startFt = clamp(num(item.startFt, 0), 0, span);
        const endFt = clamp(num(item.endFt, 0), 0, span);
        return {
          ...item,
          startFt,
          endFt,
          lengthFt: Math.max(0, num(item.lengthFt, endFt - startFt))
        };
      })
  ...
  function renderCameraLayoutSvg(model) {
    ...
    const coverageSegments = normalizeSegments(m.coverageSegments, spanFt);
    const overlapSegments = normalizeSegments(m.overlapSegments, spanFt);
    const gapSegments = normalizeSegments(m.gapSegments, spanFt);
    ...
    const gapTone = uncoveredFt > 0 ? "rgba(255,138,102,.92)" : "rgba(125,255,152,.90)";

    const cameraSvg = cameras.slice(0, 8).map((camera, index) => {
      const centerFt = num(camera.centerFt, spanFt / 2);
      const footprintStartFt = num(camera.footprintStartFt, centerFt);
      const footprintEndFt = num(camera.footprintEndFt, centerFt);

      const cx = xForFt(centerFt);
      const left = xForFt(footprintStartFt);
      const right = xForFt(footprintEndFt);
      const safeLeft = Math.max(runX, Math.min(runX + runW, left));
      const safeRight = Math.max(runX, Math.min(runX + runW, right));

      const label = camera.label || "Cam " + (index + 1);

      return '' +
        '<path d="M ' + cx.toFixed(1) + ' ' + camY + ' L ' + safeLeft.toFixed(1) + ' ' + coneY + ' L ' + safeRight.toFixed(1) + ' ' + coneY + ' Z" fill="rgba(125,255,152,.075)" stroke="rgba(125,255,152,.34)" stroke-width="1.05" />' +
        ...
        '<text x="' + cx.toFixed(1) + '" y="' + (camY - 18) + '" text-anchor="middle" fill="rgba(226,232,240,.70)" font-size="10.5" font-weight="850">' + escapeHtml(label) + '</text>';
    }).join("");
    ...
    return '' +
      '<svg data-export-svg viewBox="0 0 ' + width + ' ' + height + '" role="img" aria-label="' + escapeHtml(m.ariaLabel || "ScopedLabs camera layout visualization") + '">' +
        '<defs>' +
          '<linearGradient id="slCoveredBand" x1="0" y1="0" x2="1" y2="0">' +
            '<stop offset="0%" stop-color="rgba(82,201,112,.62)" />' +
            '<stop offset="100%" stop-color="rgba(151,255,176,.92)" />' +
          '</linearGradient>' +
          '<linearGradient id="slGreenBar" x1="0" y1="0" x2="1" y2="0">' +
            '<stop offset="0%" stop-color="rgba(84,212,116,.70)" />' +
            '<stop offset="100%" stop-color="rgba(125,255,152,.90)" />' +
          '</linearGradient>' +
          '<linearGradient id="slGapBar" x1="0" y1="0" x2="1" y2="0">' +
            '<stop offset="0%" stop-color="rgba(255,211,79,.76)" />' +
            '<stop offset="100%" stop-color="rgba(255,138,102,.90)" />' +
          '</linearGradient>' +
        '</defs>' +

        '<text x="52" y="28" fill="rgba(248,250,252,.94)" font-size="18" font-weight="950">' + escapeHtml(m.title || "Plan view: spacing, overlap, and blind gaps") + '</text>' +
        '<text x="52" y="50" fill="rgba(226,232,240,.62)" font-size="12">' + escapeHtml(m.subtitle || "Green is covered, amber is shared overlap, red is uncovered.") + '</text>' +
