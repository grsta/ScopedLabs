...
| Storage Throughput | Core storage path | Storage IOPS in guided flow; standalone allowed | Route after Storage IOPS when storage path is selected | VM Density or Summary TBD | status | requiredThroughputMBps, availableThroughputMBps, throughputUtilizationPct, headroomMBps, deficitMBps, transferWindowRequiredMBps, datasetTB, transferWindowHours, transportPath, mediaTier, workloadType, dominantConstraint, status | Contributes after valid result; final Summary gate TBD after VM Density route is proven | scripts/run-compute-storage-throughput-engineering-inputs-closeout-v1.js |
