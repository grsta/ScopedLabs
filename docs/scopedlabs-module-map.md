...
- COMPUTE_STORAGE_FLOW_NEXT_TOOL_CTA_CONTRACT_0705: Enforces Storage IOPS and Storage Throughput bottom flow CTA ownership. Back/Continue actions must render inside `#toolCard`, use split left/right layout with `#continue-wrap` at `margin-left:auto`, and display the next tool label instead of Summary. IOPS continues to Storage Throughput; Storage Throughput continues to VM Density. Audit: `scripts/audit-compute-storage-cta-next-tool-v1.js`; runner: `scripts/run-compute-storage-cta-next-tool-closeout-v1.js`.
