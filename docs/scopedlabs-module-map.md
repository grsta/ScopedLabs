...
### COMPUTE_VM_DENSITY_SHARED_ASSISTANT_RESULT_0706

- Scope: VM Density full visible shell result ownership.
- Owner: `assets/scopedlabs-compute-assistant-contract.js` renders the full VM Density planning result from `vmDensityResult`.
- Consumer: `tools/compute/vm-density/index.html` loads the bumped assistant contract cache-bust.
- Audit: `scripts/audit-compute-vm-density-shell-promotion-v1.js`.
- Guardrail: Do not hide the legacy analyzer result unless the shared assistant renders upgraded planning evidence, capacity evidence, and planner decision fields.
