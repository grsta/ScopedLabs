...
- Owner: scripts/audit-compute-gpu-vram-proof-stack-parity-v1.js
- Scope: GPU VRAM live proof-stack rendering.
- Consumer: tools/compute/gpu-vram/index.html; tools/compute/gpu-vram/script.js
- Contract: The live GPU VRAM Capacity Envelope render path must also reveal and populate Recommendation References, Recommended Actions, and GPU VRAM Decision Schedule below the chart before User Tool Notes.
- Guardrail: Do not leave proof cards only wired through an unused shell-proof path.


### COMPUTE_GPU_VRAM_PROOF_STACK_REHYDRATE_0624C

- Status: PROOF_GATE_FIX
- Owner: scripts/audit-compute-gpu-vram-proof-stack-parity-v1.js
- Scope: GPU VRAM proof-stack post-render rehydration.
- Consumer: tools/compute/gpu-vram/index.html; tools/compute/gpu-vram/script.js
- Contract: The live GPU VRAM result path must call a shared local proof-stack helper immediately after chart rendering and again after the render stack settles, so Recommendation References, Recommended Actions, and GPU VRAM Decision Schedule remain visible before User Tool Notes.
- Guardrail: Do not leave proof cards hidden or empty after Calculate when the GPU VRAM Capacity Envelope is visible.
