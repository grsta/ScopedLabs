...
- COMPUTE_STORAGE_IOPS_DECISION_SCHEDULE_SIMPLE_VALUES_0705: Updates Storage IOPS Decision Schedule rows to match RAM behavior: simple Value cells, explanatory Engineering Note cells, and a square color-coded Status badge when the value is RISK/WATCH/GOOD. Owner: `tools/compute/storage-iops/script.js`; audit owner: `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_CLOSEOUT_BATCH_RUNNER_0705: Adds a paste-friendly quiet closeout runner for the Storage IOPS lane. It runs syntax checks, chart/layout audit, planning shell audit, module map audit, and git status, then prints only overall pass/fail, failed sections with failed labels/errors, passed section names, and current git status. Owner: `scripts/run-compute-storage-iops-closeout-v1.js`.

- COMPUTE_STORAGE_IOPS_EXPORT_FOOTER_CLEANUP_0705: Aligns Storage IOPS export/footer chrome with RAM by removing the page-local `Documentation & Export` eyebrow, restoring `Export Report` to RAM-style top spacing, and tightening the Storage IOPS flow-action row under the export card. Owners: `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_FOOTER_ALIGNMENT_0705: Fixes Storage IOPS footer placement by removing the premature `</main>` before the page footer so the footer aligns with the page/card container flow. Adds page-local tightened footer top padding after the Compute flow-action row. Owners: `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_EXPORT_MATCH_ACCEPTED_SECTIONS_0705: Updates Storage IOPS export hooks so the generated report captures the accepted Capacity Envelope SVG, RAM-style Recommendation References, Recommended Actions text, Decision Schedule table/interpretation, and result summary through the shared `assets/export.js` extra-section path. Owners: `tools/compute/storage-iops/index.html`, `tools/compute/storage-iops/script.js`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_EXPORT_INTERPRETATION_CLEANUP_0705: Replaces the Storage IOPS export interpretation data blob with a concise report-readable Engineering Interpretation section sourced from `#analysis-copy`, using structured Status, Why it matters, Primary constraint, Recommended correction, and Carry forward lines while preserving the shared export engine. Owners: `tools/compute/storage-iops/script.js`, `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_EXPORT_ANALYZER_SOURCE_SUPPRESSION_0705: Suppresses the hidden analyzer `#results` interpretation headings before Storage IOPS export capture so the generated report uses only the concise structured Engineering Interpretation instead of appending it after the legacy analyzer dump. Owners: `tools/compute/storage-iops/script.js`, `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_CUSTOM_EXPORT_PAYLOAD_0705: Gives Storage IOPS a CPU-style custom export payload builder through `ScopedLabsComputeStorageIopsExport.buildPayload`, bypassing shared analyzer fallback interpretation and exporting the accepted Capacity Envelope, Recommendation References, Recommended Actions, Decision Schedule, and concise report-readable interpretation. Owners: `tools/compute/storage-iops/index.html`, `tools/compute/storage-iops/script.js`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_CUSTOM_EXPORT_DATA_FIX_0705: Fixes Storage IOPS custom export data resolution so the report interpretation reads required IOPS, available IOPS, utilization, latency target, and block size from the accepted result rows/Decision Schedule when the visual result object does not expose those fields directly. Owners: `tools/compute/storage-iops/script.js`, `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_STORAGE_IOPS_EXPORT_INTERPRETATION_LAYOUT_0705: Moves Storage IOPS export Engineering Interpretation out of the default single-paragraph report block and into a custom extra section after the Decision Schedule, preserving line breaks for Status, Why it matters, Primary constraint, Recommended correction, and Carry forward. Owners: `tools/compute/storage-iops/script.js`, `tools/compute/storage-iops/index.html`, `scripts/audit-compute-storage-iops-planning-shell-v1.js`.

- COMPUTE_PLANNER_UPDATE_LEDGER_0705: Source-of-truth planning ledger for future Compute planner/guided-flow work. Tracks each Compute tool's role, upstream dependency, downstream route, planner-visible status, carry-forward fields, Summary rule, and closeout runner so planner routing can be updated once at the end instead of revisiting every tool. Owner: `docs/compute-planner-update-ledger.md`.

- COMPUTE_STORAGE_THROUGHPUT_ENGINEERING_INPUTS_0705: Upgrades Storage Throughput from a thin IOPS/block-size estimator into a planner-ready throughput model with available path ceiling, burst multiplier, growth reserve, dataset size, transfer window, transport path, media tier, workload type, utilization, headroom/deficit, transfer-window required MB/s, and carry-forward fields for the future Compute planner lane. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-engineering-inputs-v1.js`, `scripts/run-compute-storage-throughput-engineering-inputs-closeout-v1.js`, `docs/compute-planner-update-ledger.md`.

- COMPUTE_STORAGE_THROUGHPUT_CAPACITY_VISUAL_0705: Adds shared Compute Capacity Visuals support for Storage Throughput Capacity Envelope rendering, including MB/s demand curve, visible GOOD/WATCH/RISK zone bands, burst/required markers, available path ceiling, headroom/deficit bracket, status badge, and throughput planning chips. Owners: `assets/scopedlabs-compute-capacity-visuals.js`, `scripts/audit-compute-storage-throughput-capacity-visual-v1.js`, `scripts/run-compute-storage-throughput-capacity-visual-closeout-v1.js`.

- COMPUTE_STORAGE_THROUGHPUT_VISUAL_WIRE_0705: Wires the Storage Throughput page to the shared Compute Capacity Visuals Storage Throughput Capacity Envelope renderer. Adds the page visual card, export SVG mount, shared visual asset include, visual clear/render hooks, legacy analyzer chart suppression, and focused visual-wire closeout audit. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-visual-wire-v1.js`, `scripts/run-compute-storage-throughput-visual-wire-closeout-v1.js`.

- COMPUTE_CAPACITY_ZONE_BAND_CONTRACT_0705: Promotes GOOD/WATCH/RISK plot background bands into reusable Compute Capacity Visuals helpers so future Compute Capacity Envelope charts can call `buildCapacityZoneBands(plot, yGood, yWatch)` and `computeCapacityZoneBandStyles()` instead of recreating one-off band rectangles/styles. First consumer: Storage Throughput Capacity Envelope. Owners: `assets/scopedlabs-compute-capacity-visuals.js`, `scripts/audit-compute-storage-throughput-capacity-visual-v1.js`, `scripts/run-compute-storage-throughput-capacity-visual-closeout-v1.js`.

- COMPUTE_CAPACITY_INLINE_ICON_LIBRARY_0705: Promotes accepted Storage IOPS inline footer icons into a reusable Compute Capacity Visuals icon/chip helper. Shared helpers include `computeCapacityIconSvg(iconKey, x, y)`, `computeCapacityFooterIconStyles()`, and `buildCapacityFooterStat(...)`. Seeded icon keys: storage, workload, raid, latency, block, network, media, utilization. First new consumer: Storage Throughput footer chips. Owner: `assets/scopedlabs-compute-capacity-visuals.js`; audit: `scripts/audit-compute-storage-throughput-capacity-visual-v1.js`.

- COMPUTE_CAPACITY_GUIDE_LINE_CONTRACT_0705: Promotes Compute Capacity Envelope guide-line styling into the shared visual module. Adds reusable full-height checkpoint guide lines via `buildCapacityCheckpointGuides(plot, checkpoints)` and a white dashed ceiling/threshold style via `computeCapacityGuideLineStyles()`. First consumer: Storage Throughput Capacity Envelope. Owner: `assets/scopedlabs-compute-capacity-visuals.js`; audit: `scripts/audit-compute-storage-throughput-capacity-visual-v1.js`.

- COMPUTE_STORAGE_THROUGHPUT_PROOF_STACK_0705: Adds visible Storage Throughput Recommendation References, Recommended Actions, and Decision Schedule sections after the Capacity Envelope visual. Wires section clear/render helpers and payload fields for downstream export wiring. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-proof-stack-v1.js`, `scripts/run-compute-storage-throughput-proof-stack-closeout-v1.js`.

- COMPUTE_STORAGE_THROUGHPUT_EXPORT_PAYLOAD_0705: Adds custom Storage Throughput export payload wiring for the Capacity Envelope SVG, Recommendation References, Recommended Actions, Decision Schedule, and Engineering Interpretation sections. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-export-payload-v1.js`, `scripts/run-compute-storage-throughput-export-payload-closeout-v1.js`.

- COMPUTE_STORAGE_THROUGHPUT_SHELL_PARITY_0705: Aligns Storage Throughput with the accepted Compute shell foundation after visual/proof/export completion. Hides the legacy analyzer Results card behind an internal ledger, adds a visible result summary shell, adds assistant/report metadata/export shell structure, replaces the old Documentation & Export pill card, adds compute flow actions, and loads Compute shell support scripts. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-shell-parity-v1.js`, `scripts/run-compute-storage-throughput-shell-parity-closeout-v1.js`.
...
- COMPUTE_POWER_THERMAL_VISUAL_ENVELOPE_0708: Adds the shared Power / Thermal Infrastructure Envelope visual after the modern planning-shell pass. Accepted reference source is Storage Throughput for shared Capacity Envelope rhythm and VM Density for latest Compute specialty visual parity. Visual owner: `assets/scopedlabs-compute-capacity-visuals.js` through `buildPowerThermalCapacityEnvelopeSvg` and `renderPowerThermalCapacityEnvelope`. Page/script wiring owners: `tools/compute/power-thermal/index.html` and `tools/compute/power-thermal/script.js`. Export owner remains the modern Power / Thermal custom payload/shared export route with the visual exposed through the export SVG mount. Route owner remains the shared planner/shell state; do not hardcode RAID. Audit owner: `scripts/audit-compute-power-thermal-visual-envelope-v1.js`. Non-goals: no math rewrite, no auth/checkout/export/snapshot rewrite, no planner route rewrite, no Power / Thermal shell rhythm restart.
