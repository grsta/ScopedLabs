...
### COMPUTE_GPU_VRAM_PROOF_STACK_ACTIVE_RENDER_0624E

- Status: PROOF_GATE_FIX
- Owner: scripts/audit-compute-gpu-vram-proof-stack-parity-v1.js
- Scope: GPU VRAM active engineering renderer proof-stack call.
- Consumer: tools/compute/gpu-vram/index.html; tools/compute/gpu-vram/script.js
- Contract: The actual active GPU renderer, renderGpuEngineeringPlan(), must call renderGpuProofSections(plan) immediately after envelope.innerHTML = envelopeSvg(plan). The audit must check the renderGpuEngineeringPlan function block, not global script order.
- Guardrail: Do not accept global token-order proof for live render wiring.
