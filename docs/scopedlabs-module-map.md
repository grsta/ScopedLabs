...
- COMPUTE_STORAGE_THROUGHPUT_ENGINEERING_INPUTS_0705: Upgrades Storage Throughput from a thin IOPS/block-size estimator into a planner-ready throughput model with available path ceiling, burst multiplier, growth reserve, dataset size, transfer window, transport path, media tier, workload type, utilization, headroom/deficit, transfer-window required MB/s, and carry-forward fields for the future Compute planner lane. Owners: `tools/compute/storage-throughput/index.html`, `tools/compute/storage-throughput/script.js`, `scripts/audit-compute-storage-throughput-engineering-inputs-v1.js`, `docs/compute-planner-update-ledger.md`.
