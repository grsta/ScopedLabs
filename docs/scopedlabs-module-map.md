...
- COMPUTE_STORAGE_THROUGHPUT_CAPACITY_VISUAL_0705: Adds shared Compute Capacity Visuals support for Storage Throughput Capacity Envelope rendering, including MB/s demand curve, burst/required markers, available path ceiling, headroom/deficit bracket, status badge, and throughput planning chips. Owners: `assets/scopedlabs-compute-capacity-visuals.js`, `scripts/audit-compute-storage-throughput-capacity-visual-v1.js`, `scripts/run-compute-storage-throughput-capacity-visual-closeout-v1.js`.
