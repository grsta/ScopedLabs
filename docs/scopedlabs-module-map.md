...
- `assets/scopedlabs-compute-capacity-visuals.js` now scales the CPU Capacity Envelope around demand curve plus usable capacity instead of letting raw recommended logical cores dominate the Y-axis.
- Low target-utilization cases such as 10% can still show `Recommended - N logical cores` as an above-scale callout while preserving a readable demand/usable-capacity graph.
- CPU/RAM pages that load the shared capacity visual are cache-busted to `scopedlabs-compute-capacity-visuals-012-cpu-readable-scale`.
- `scripts/audit-compute-cpu-capacity-envelope-scale-v1.js` verifies the low-utilization/high-recommendation case and readable tick density.
