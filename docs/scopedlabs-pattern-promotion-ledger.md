...
### COMPUTE-GPU-VRAM-PROOF-STACK-LIFECYCLE-0624D
...
Fix: GPU VRAM proof stack now uses a CPU-style lifecycle: clearGpuProofSections() and renderGpuProofSections(plan). Proof-card lookup uses a local DOM resolver so cards outside the shell search scope can be populated reliably.

Audit:
- scripts/audit-compute-gpu-vram-proof-stack-parity-v1.js


### COMPUTE-GPU-VRAM-PROOF-STACK-LIFECYCLE-AUDIT-ALIGNMENT-0624D

Status: LOCAL_PROOF_GATE_AUDIT_ALIGNMENT
...
Audit lineage tokens preserved:
- COMPUTE-GPU-VRAM-PROOF-STACK-PARITY-0624A
