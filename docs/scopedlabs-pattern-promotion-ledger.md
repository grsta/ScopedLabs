...
### COMPUTE-VM-DENSITY-SHELL-PROMOTION-0706

Status: SHELL_PROMOTION_READY

Changed path:
- tools/compute/vm-density/index.html
- tools/compute/vm-density/script.js
- scripts/audit-compute-vm-density-shell-promotion-v1.js
- scripts/run-compute-vm-density-shell-promotion-closeout-v1.js
- docs/scopedlabs-module-map.md
- docs/scopedlabs-pattern-promotion-ledger.md

Decision:
- VM Density now opts into the shared Compute shell using a VM Density shell-promotion marker and an explicit Power / Thermal continuation target.
- The old design-flow explainer is now a shell-addressable Active Workflow card, matching the Storage Throughput forward placement contract without duplicating the help.js KB guide card.
- The legacy analyzer Results block remains available as an internal ledger but is hidden from the visible output flow.
- The local script keeps upstream Storage Throughput context for storage-pressure prefill and payload evidence while suppressing visible Flow Context output.
- After Calculate, the shared assistant card is inserted and rendered before the capacity visual so the assistant is the first visible result card.
- Continue now routes to Power / Thermal; GPU VRAM remains a specialty branch candidate in the VM Density payload.
