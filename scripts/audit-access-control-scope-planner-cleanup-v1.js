...
check("Planner version bumped to card polish lane", index.includes("access-control-scope-planner-area-match-004-card-polish"));
...
check(
  "Bad display separators are not present",
  !script.includes("' ? '") &&
  !script.includes('" ? "') &&
  !script.includes("' ? '") &&
  !script.includes('" ? "') &&
