...
const auditVersion = "blind-spot-guidance-event-bridge-proof-audit-001";

const indexFile = path.join(root, "tools", "physical-security", "blind-spot-check", "index.html");
const scriptFile = path.join(root, "tools", "physical-security", "blind-spot-check", "script.js");
...
const rows = [
  {
    ...
    detail: "Blind Spot index exists"
  },
  {
    id: "memory-loaded",
    ...
    detail: "Blind Spot loads shared guidance memory"
  },
  {
    ...
    detail: "Blind Spot loads current memory-aware category guidance"
  },
  {
    id: "bridge-loaded",
    ...
    detail: "Blind Spot loads shared guidance event bridge"
  },
  {
    id: "bridge-singleton",
  ...
  {
    id: "local-cache",
    status: html.includes("./script.js?v=blindspot-guidance-event-bridge-001-nonvisible") ? "SAFE" : "WATCH",
    detail: "Blind Spot local script cache bumped"
  },
  {
    id: "adapter-global-preserved",
    status: js.includes("ScopedLabsBlindSpotGuidance") && js.includes("getLastGuidance") ? "SAFE" : "WATCH",
    detail: "Blind Spot guidance adapter global remains available"
  },
  {
    id: "publish-helper",
    status:
      js.includes("function publishBlindSpotGuidanceEvent") &&
      js.includes("ScopedLabsPhysicalSecurityGuidanceEventBridge") &&
      js.includes("publishIfChanged") &&
      js.includes('tool: "blind-spot-check"') ? "SAFE" : "WATCH",
    detail: "Blind Spot can publish validated guidance through event bridge"
  },
  {
    id: "publish-hook",
    status:
      js.includes("updateBlindSpotUserGuidance(data);") &&
      js.includes('publishBlindSpotGuidanceEvent("blind-spot-guidance-update")') ? "SAFE" : "WATCH",
    detail: "Blind Spot publishes after validated guidance update"
  },
  {
    id: "raw-edit-clear",
    status:
      js.includes("function clearBlindSpotGuidanceEventMemory") &&
      js.includes('bridge.clearTool("blind-spot-check")') &&
      js.includes("clearBlindSpotGuidanceEventMemory();") ? "SAFE" : "WATCH",
    detail: "Blind Spot clears stale memory on raw edits"
  },
  {
    id: "existing-assistant-preserved",
    status: js.includes("renderBlindSpotAssistant(data);") && js.indexOf("renderBlindSpotAssistant(data);") < js.indexOf("writeFlow(data);") ? "SAFE" : "WATCH",
    detail: "existing visible Blind Spot assistant remains before writeFlow"
  },
  {
    ...
    detail: "Blind Spot does not add visible category renderer"
  },
  {
  ...
  },
  {
    ...
    detail: "Blind Spot event bridge proof adds no runtime web fetch"
  }
];

console.log("\nBlind Spot Guidance Event Bridge Proof Audit\n");
