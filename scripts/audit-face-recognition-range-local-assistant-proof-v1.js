...
const indexRel = "tools/physical-security/face-recognition-range/index.html";
const scriptRel = "tools/physical-security/face-recognition-range/script.js";
...
add(checks, "index-exists", index ? "SAFE" : "FAIL", index ? "Face Recognition Range index exists" : "Missing Face Recognition Range index");
add(checks, "script-exists", localScript ? "SAFE" : "FAIL", localScript ? "Face Recognition Range script exists" : "Missing Face Recognition Range script");
...
[
  ...
  add(checks, "loads-" + asset.replace(/[^a-z0-9]+/gi, "-").replace(/-$/, ""), has(index, asset) ? "SAFE" : "FAIL", has(index, asset) ? "Face Recognition Range loads " + asset : "Face Recognition Range missing " + asset);
});

add(checks, "visible-mount-present", has(index, 'id="' + MOUNT_ID + '"') || has(index, "id='" + MOUNT_ID + "'") ? "SAFE" : "FAIL", "Face Recognition Range has dedicated local assistant mount");
add(checks, "local-script-cache-bumped", has(index, "./script.js?v=" + VERSION) ? "SAFE" : "FAIL", "Face Recognition Range local script cache points to " + VERSION);
...
[
  ...
  "faceRecognitionRangeLocalAssistantProof",
  "buildModel",
  "render",
  "clear"
...
add(checks, "category-renderer-still-not-on-tool", !has(index, "physical-security-category-guidance-renderer.js") ? "SAFE" : "FAIL", "Face Recognition Range does not load visible category/master renderer");
add(checks, "report-summary-still-not-on-tool", !has(index, "physical-security-report-summary.js") ? "SAFE" : "FAIL", "Face Recognition Range does not load category report summary helper");
...
console.log("");
console.log("Face Recognition Range Local Assistant Proof Audit");
