...
const auditVersion = "scene-illumination-guidance-event-bridge-proof-audit-001";

const indexFile = path.join(root, "tools", "physical-security", "scene-illumination", "index.html");
const scriptFile = path.join(root, "tools", "physical-security", "scene-illumination", "script.js");
...
const rows = [
  {
    id: "scene-illumination-index",
    status: fs.existsSync(indexFile) ? "SAFE" : "FAIL",
    detail: "Scene Illumination index exists"
  },
  {
    id: "memory-loaded",
    ...
    detail: "Scene Illumination loads shared guidance memory"
  },
  {
    ...
    detail: "Scene Illumination loads current memory-aware category guidance"
  },
  {
    id: "bridge-loaded",
    ...
    detail: "Scene Illumination loads shared guidance event bridge"
  },
  {
    id: "bridge-singleton",
  ...
  {
    id: "local-cache",
    status: html.includes("./script.js?v=physical-security-scene-guidance-event-bridge-001-nonvisible") ? "SAFE" : "WATCH",
    detail: "Scene Illumination local script cache bumped"
  },
  {
    id: "adapter-global-preserved",
    status: js.includes("ScopedLabsSceneIlluminationGuidance") && js.includes("getLastGuidance") ? "SAFE" : "WATCH",
    detail: "Scene Illumination guidance adapter global remains available"
  },
  {
    id: "publish-helper",
    status:
      js.includes("function publishSceneIlluminationGuidanceEvent") &&
      js.includes("ScopedLabsPhysicalSecurityGuidanceEventBridge") &&
      js.includes("publishIfChanged") &&
      js.includes('tool: "scene-illumination"') ? "SAFE" : "WATCH",
    detail: "Scene Illumination can publish validated guidance through event bridge"
  },
  {
    id: "publish-hook",
    status:
      js.includes("updateSceneIlluminationUserGuidance(data);") &&
      js.includes('publishSceneIlluminationGuidanceEvent("scene-illumination-guidance-update")') ? "SAFE" : "WATCH",
    detail: "Scene Illumination publishes after validated guidance update"
  },
  {
    id: "raw-edit-clear",
    status:
      js.includes("function clearSceneIlluminationGuidanceEventMemory") &&
      js.includes('bridge.clearTool("scene-illumination")') &&
      js.includes("clearSceneIlluminationGuidanceEventMemory();") ? "SAFE" : "WATCH",
    detail: "Scene Illumination clears stale memory on raw edits"
  },
  {
    ...
    detail: "Scene Illumination does not add visible category renderer"
  },
  {
  ...
  },
  {
    ...
    detail: "Scene Illumination event bridge proof adds no runtime web fetch"
  }
];

console.log("\nScene Illumination Guidance Event Bridge Proof Audit\n");
