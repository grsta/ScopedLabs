...
if (!TARGET) {
  console.error("Usage:");
  console.error("  node scripts/export-force-install-flex.js tools/video-storage/retention-planner/index.html --write --version shared-export-004");
  process.exit(1);
}
...
function findMatchingTag(html, openStart, tagName) {
  ...
  while (true) {
    ...
    if (!isClose) depth += 1;
      ...
      depth -= 1;
      if (depth === 0) return { start: openStart, closeStart: m.index, end: m.index + m[0].length };
    }
  }
}
...
function findElementById(html, id) {
  ...
  return findMatchingTag(html, m.index, m[1].toLowerCase());
}
...
function buildConfig(categorySlug, toolSlug, toolLabel, inputSelector) {
  const config = {
    ...
    inputContainerSelector: inputSelector,
    assumptions: [
...
const inputSelector = /id=["']toolCard["']/i.test(html) ? "#toolCard" : "main";
...
if (!/id=["']exportReport["']/i.test(html)) {
  ...
  const insertAfter = resultsSection || resultsElement;

  if (!insertAfter) throw new Error("Could not find #results block");

  html = html.slice(0, insertAfter.end) + "\n" + buildCard(toolLabel) + "\n" + html.slice(insertAfter.end);
}

if (!html.includes("window.ScopedLabsExportConfig")) {
  html = html.replace(
    /(\s*)<script\b([^>]*?)src=["']\.\/script\.js(?:\?[^"']*)?["']([^>]*)><\/script>/i,
    `${buildConfig(categorySlug, toolSlug, toolLabel, inputSelector)}\n$1<script src="/assets/export.js?v=${VERSION}"></script>\n$&`
  );
