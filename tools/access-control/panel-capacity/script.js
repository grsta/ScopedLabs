// access-control-panel-capacity-export-print-ux-001: local print report wording and page-pack polish.
(() => {
  "use strict";
  ...
  function svgDataUri(svg) {
    ...
    if (visuals && typeof visuals.renderPanelCapacity === "function") {
      return visuals.renderPanelCapacity({
        card: els.visualCard,
        wrap: els.chartWrap,
        target: els.chart,
        metrics
    ...
    const svg = visuals && typeof visuals.buildPanelCapacitySvg === "function"
      ? visuals.buildPanelCapacitySvg(metrics || {}, { exportMode: false })
      : "";

    if (svg && shell && typeof shell.showVisual === "function") {
      return shell.showVisual({ card: els.visualCard, wrap: els.chartWrap, target: els.chart, html: svg });
    }

    if (els.chart) els.chart.innerHTML = svg;
    if (els.chartWrap) els.chartWrap.hidden = !svg;
    if (els.visualCard) els.visualCard.hidden = !svg;
    return !!svg;
  }
  ...
  function getPanelCapacityVisualImage(metrics, options = {}) {
    ...
    const visuals = window.ScopedLabsAccessControlPlanningVisuals;
    if (!visuals || typeof visuals.buildPanelCapacitySvg !== "function") return getChartImage();

    const svg = visuals.buildPanelCapacitySvg(metrics, Object.assign({ exportMode: false }, options));
    return svg ? svgDataUri(svg) : getChartImage();
  }
      ...
      ? `
        <section class="section">
          ...
          <div class="chart-wrap">
            <img src="${payload.chartImage}" alt="Panel Capacity Planner planning visual">
          </div>
        </section>
      `
      : "";
