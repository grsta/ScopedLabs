(() => {
  ...
  // power-thermal-action-delegate-final-0709
  function resetPowerThermalInputsFromDefaults0709() {
    [
      "nodes",
      "watts",
      "peak",
      ...
      "circuitAmps",
      "coolingTons",
      "coolingTonsAvailable"
    ].forEach(function (id) {
      const el = $(id);
      if (!el) return;

      if (el.tagName === "SELECT") {
        const selected = Array.prototype.find.call(el.options || [], function (option) {
          return option.defaultSelected;
        }) || el.options && el.options[0];
  ...
  function bindPowerThermalActionDelegate0709() {
    if (document.documentElement.dataset.powerThermalActionDelegateFinal0709 === "true") return;
    document.documentElement.dataset.powerThermalActionDelegateFinal0709 = "true";
    ...
    document.addEventListener("click", function (event) {
      ...
      if (target.closest("#reset")) {
        event.preventDefault();
        event.stopImmediatePropagation();
        resetPowerThermalInputsFromDefaults0709();
      }
    }, true);
  }

  bindPowerThermalActionDelegate0709();
  window.addEventListener("DOMContentLoaded", bindPowerThermalActionDelegate0709);
