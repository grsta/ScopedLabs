# MTU & Fragmentation Helper

Calculates maximum payload size based on MTU and protocol headers to identify when fragmentation will occur.

## Status
- Page created
- Inputs wired
- Math logic: TODO
- Results displaying

## Files
- \index.html\ â€” tool UI (ScopedLabs template)
- \script.js\ â€” calculation logic + events
- \ccess.json\ â€” tool metadata + access tier

## Notes
- Run from project root: visit \/tools/network/mtu-fragmentation/\
"@

# Seed access.json
# (Write as pretty JSON without needing external modules)
 = ""
if (.Count -gt 0) {
   =  | ForEach-Object { '"' + ( -replace '"','\"') + '"' }
   = "  ""tags"": [ " + ( -join ", ") + " ],
"
} else {
   = "  ""tags"": [],
"
}

 = @"
{
  "slug": "mtu-fragmentation",
  "title": "MTU & Fragmentation Helper",
  "category": "network",
  "access": "Free",
  "version": "0.1.0",
  "created": "2026-02-04T22:28:40"
}
