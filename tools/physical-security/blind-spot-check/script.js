...
/* ScopedLabs Blind Spot Check Local Assistant Proof
   Version: blind-spot-check-local-assistant-proof-001
   Purpose: visible local assistant proof for Blind Spot Check only.
   Notes:
...
*/
(function blindSpotCheckLocalAssistantProof() {
  "use strict";

  const VERSION = "blind-spot-check-local-assistant-proof-001";
  const TOOL_SLUG = "blind-spot-check";
  const TOOL_TEXT = "blind spot";
  ...
  function readSummary(record) {
    return asText(
      ...
      record.description ||
      record.primaryText
    ) || "Blind spot coverage has been evaluated. Review obstruction, coverage gap, and camera placement assumptions before carrying this area forward.";
  }
  ...
  function readActions(record) {
    ...
    if (!actions.length) {
      actions.push("Confirm the modeled blind spot or obstruction condition before continuing into pixel density and final camera placement checks.");
    }
  ...
  function buildModel(recordInput) {
    ...
    return {
      tool: TOOL_SLUG,
      title: adapter && adapter.title ? adapter.title : "Blind Spot Assistant",
      status: readStatus(record),
      summary: readSummary(record),
      assumptions: readAssumptions(record),
      actions: readActions(record),
      iconKey: adapter && adapter.iconKey ? adapter.iconKey : "blindSpot",
      visible: true
    };
  }
  ...
  window.ScopedLabsBlindSpotCheckLocalAssistantProof = Object.freeze({
    version: VERSION,
