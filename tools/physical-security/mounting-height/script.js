...
/* ScopedLabs Mounting Height Local Assistant Proof
   Version: mounting-height-local-assistant-proof-001
   Purpose: visible local assistant proof for Mounting Height only.
   Notes:
...
*/
(function mountingHeightLocalAssistantProof() {
  "use strict";

  const VERSION = "mounting-height-local-assistant-proof-001";
  const TOOL_SLUG = "mounting-height";
  const TOOL_TEXT = "mounting height";
  ...
  function readSummary(record) {
    return asText(
      ...
      record.description ||
      record.primaryText
    ) || "Mounting height has been evaluated. Review the camera height assumptions before carrying this area into field-of-view and coverage checks.";
  }
  ...
  function readActions(record) {
    ...
    if (!actions.length) {
      actions.push("Confirm the mounting height assumption before continuing into field-of-view and camera coverage planning.");
    }
  ...
  function buildModel(recordInput) {
    ...
    return {
      tool: TOOL_SLUG,
      title: adapter && adapter.title ? adapter.title : "Mounting Height Assistant",
      status: readStatus(record),
      summary: readSummary(record),
      assumptions: readAssumptions(record),
      actions: readActions(record),
      iconKey: adapter && adapter.iconKey ? adapter.iconKey : "camera",
      visible: true
    };
  }
  ...
  window.ScopedLabsMountingHeightLocalAssistantProof = Object.freeze({
    version: VERSION,
