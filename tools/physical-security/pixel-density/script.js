...
/* ScopedLabs Pixel Density Local Assistant Proof
   Version: pixel-density-local-assistant-proof-001
   Purpose: visible local assistant proof for Pixel Density only.
   Notes:
...
*/
(function pixelDensityLocalAssistantProof() {
  "use strict";

  const VERSION = "pixel-density-local-assistant-proof-001";
  const TOOL_SLUG = "pixel-density";
  const TOOL_TEXT = "pixel density";
  ...
  function readSummary(record) {
    return asText(
      ...
      record.description ||
      record.primaryText
    ) || "Pixel density has been evaluated. Review target pixels-per-foot, distance, scene width, and image-quality assumptions before final camera selection.";
  }
  ...
  function readActions(record) {
    ...
    if (!actions.length) {
      actions.push("Confirm the pixel density target and scene width before carrying this area into final camera/lens selection.");
    }
  ...
  function buildModel(recordInput) {
    ...
    return {
      tool: TOOL_SLUG,
      title: adapter && adapter.title ? adapter.title : "Pixel Density Assistant",
  ...
  window.ScopedLabsPixelDensityLocalAssistantProof = Object.freeze({
    version: VERSION,
