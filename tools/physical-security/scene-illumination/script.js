...
/* ScopedLabs Scene Illumination Local Assistant Proof
   Version: scene-illumination-local-assistant-proof-001
   Purpose: visible local assistant proof for Scene Illumination only.
   Notes:
...
*/
(function sceneIlluminationLocalAssistantProof() {
  "use strict";

  const VERSION = "scene-illumination-local-assistant-proof-001";
  const TOOL_SLUG = "scene-illumination";
  ...
  function eventBelongsToScene(detail) {
    const data = detail || {};
    ...
    return haystack.includes(TOOL_SLUG) || haystack.includes("scene illumination");
  }
  ...
  function readSummary(record) {
    return asText(
      ...
      record.description ||
      record.primaryText
    ) || "Scene illumination has been evaluated. Review the assumptions and recommended actions before continuing.";
  }
  ...
  function readActions(record) {
    ...
    if (!actions.length) {
      actions.push("Confirm the lighting assumption before carrying this area into camera placement and image quality checks.");
    }
  ...
  function buildModel(recordInput) {
    ...
    return {
      tool: TOOL_SLUG,
      title: adapter && adapter.title ? adapter.title : "Scene Illumination Assistant",
      status: readStatus(record),
      summary: readSummary(record),
      assumptions: readAssumptions(record),
      actions: readActions(record),
      iconKey: adapter && adapter.iconKey ? adapter.iconKey : "light",
      visible: true
    };
  }
  ...
  window.ScopedLabsSceneIlluminationLocalAssistantProof = Object.freeze({
    version: VERSION,
