(() => {
  ...
  function prefillFromPipeline() {
    ...
    if (incoming.step === "va-watts-amps") {
      ...
      if (Number.isFinite(watts)) lines.push(`Load: ${fmt(watts, 0)} W`);
      if (Number.isFinite(kw)) lines.push(`Converted load: ${fmt(kw, 3)} kW`);
      if (Number.isFinite(volts)) lines.push(`Voltage: ${fmt(volts, 0)} V`);
      if (Number.isFinite(pf)) lines.push(`Power factor: ${fmt(pf, 2)}`);
  ...
  function reset() {
    ...
    const flowNote = $("flowNote");
    if (flowNote) flowNote.hidden = true;
  }
