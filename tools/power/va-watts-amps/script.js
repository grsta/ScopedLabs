  ...
  function interpretation(payload) {
    ...
    if (Number.isFinite(utilizationPct80)) {
      if (utilizationPct80 > 100) {
        messages.push(
          `At ${fmtNum(volts)} V, this load exceeds a simple 80% continuous-planning threshold for a ${fmtNum(amps, 2)} A-equivalent draw.`
        );
      } else if (utilizationPct80 > 80) {
        messages.push("This load is approaching typical continuous-use planning limits. Leave room for startup surge and future expansion.");
      } else {
        messages.push("This load is moderate for branch planning, but design should still include future growth and inverter/UPS losses.");
      }
    }

    messages.push(
      `Planning reference values: +20% growth headroom ≈ ${fmtNum(designWatts20)} W, and a 125% continuous-design load ≈ ${fmtNum(continuousWatts125)} W.`
    );

    messages.push(
      `At an 80% continuous-load planning threshold, a nominal branch-equivalent load would be about ${fmtNum(branch80Watts)} W at ${fmtNum(volts)} V.`
    );
